...
Welcome! In this workshop you will build a **Tethys Component App** from scratch — a web app whose entire interface is written in Python using Tethys's component framework (no HTML, no JavaScript, no CSS).

## What you'll build

The **Runoff Depth Calculator** estimates stormwater runoff for an urban area using the NRCS Curve Number method. The finished app lets a user:
...
- A working Tethys Platform install. If you don't have one, complete the
  [Tethys Quick Start](https://docs.tethysplatform.org/en/stable/#quick-start) first.
- Basic Python familiarity. No web development experience required.
